/**
 * Email Validation Function v3 (Final)
 * Generated from prompt v3
 *
 * Production-ready TypeScript email validator
 ...
 * - Local part: 1-64 characters, alphanumeric start/end
 * - Total length: max 254 characters (RFC 5321)
 * - No whitespace anywhere
 * - Domain must have dot-separated parts ending in 2+ letter TLD
 */
function validateEmail(email: string): boolean {
  ...
  // ASCII only check (reject unicode)
  // eslint-disable-next-line no-control-regex
  if (!/^[\x00-\x7F]*$/.test(email)) {
    return false;
  }

  // No whitespace anywhere (space, tab, newline, carriage return, etc.)
  if (/\s/.test(email)) {
    return false;
  }
  ...
  // Must end with alphanumeric (not special char like +, -, _, .)
  if (!/[a-zA-Z0-9]$/.test(localPart)) {
    return false;
  }
...
function runTests(): boolean {
  const testCases: TestCase[] = [
    ...
    // Plus addressing
    { email: "user+newsletter@example.com", expected: true, description: "Plus addressing" },
    { email: "user+tag+another@example.com", expected: true, description: "Multiple plus signs" },

    // Subdomains
    { email: "user@mail.domain.com", expected: true, description: "Single subdomain" },
    { email: "firstname.lastname@subdomain.example.org", expected: true, description: "Complex subdomain" },
    { email: "user@a.b.c.d.example.com", expected: true, description: "Multiple subdomains" },

    // Short domains
    { email: "user@a.co", expected: true, description: "Single char domain part" },
    { email: "user@ab.co", expected: true, description: "Two char domain part" },

    // Various valid local parts
    ...
    { email: "user.name.more@example.com", expected: true, description: "Multiple dots in local" },
    ...
    // RFC 5321 limits
    { email: "a".repeat(64) + "@example.com", expected: true, description: "64 char local (max)" },

    // Complex valid
    { email: "test.email+alex@example.com", expected: true, description: "Dot and plus combo" },
    { email: "1234567890@example.com", expected: true, description: "All numeric local" },
    { email: "email@example-one.com", expected: true, description: "Hyphen in domain middle" },
    ...
    // Consecutive dots
    { email: "user..double@example.com", expected: false, description: "Double dots in local" },
    { email: "user@example..com", expected: false, description: "Double dots in domain" },

    // Leading/trailing dots
    { email: ".leadingdot@example.com", expected: false, description: "Leading dot in local" },
    { email: "trailingdot.@example.com", expected: false, description: "Trailing dot in local" },
    { email: "user@.example.com", expected: false, description: "Leading dot in domain" },
    { email: "user@example.com.", expected: false, description: "Trailing dot in domain" },

    // Special chars at boundaries
    { email: "user+@example.com", expected: false, description: "Trailing plus before @" },
    { email: "user-@example.com", expected: false, description: "Trailing hyphen before @" },
    { email: "user_@example.com", expected: false, description: "Trailing underscore before @" },
    ...
    { email: "user@-invalid.com", expected: false, description: "Domain starts with hyphen" },
    { email: "user@invalid-.com", expected: false, description: "Domain part ends with hyphen" },
    ...
    { email: "user\r@example.com", expected: false, description: "Carriage return in email" },
    { email: "\tuser@example.com", expected: false, description: "Leading tab" },
    { email: "user@example.com\n", expected: false, description: "Trailing newline" },

    // RFC 5321 violations
    { email: "a".repeat(65) + "@example.com", expected: false, description: "65 char local (exceeds)" },
    { email: "a".repeat(64) + "@" + "b".repeat(250) + ".com", expected: false, description: "Exceeds 254 total" },

    // Unicode/special chars
    { email: "user@éxample.com", expected: false, description: "Unicode in domain" },
    { email: "üser@example.com", expected: false, description: "Unicode in local" },
    { email: "user@example.рф", expected: false, description: "Cyrillic TLD" },
    ...
    // Other invalid
    { email: '"quoted"@example.com', expected: false, description: "Quoted local part" },
    { email: "email@example.com (comment)", expected: false, description: "Comment after email" },
  ];

  console.log("Email Validator v3 (Final) - Test Suite");
