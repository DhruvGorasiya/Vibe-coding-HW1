/**
 * Cache v2 - O(1) LRU with doubly-linked list, debounced persistence, batch ops
 */
...
export class LRUCache<T = unknown> {
  ...
  private opts: Required<Omit<CacheOptions<T>, "onEvict" | "onExpire">> & Pick<CacheOptions<T>, "onEvict" | "onExpire">;
  ...
  constructor(options: CacheOptions<T>) {
    this.opts = {
      ...
      storageKey: options.storageKey ?? "lru_cache_v2",
  ...
  touch(key: string, ttl?: number | null): boolean {
    ...
    if (!node || this.isExpired(node)) {
      if (node) this.removeNode(node, key, true);
      return false;
    }
